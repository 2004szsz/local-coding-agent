...
**只保留三档，且不提供「完全访问」**。理由是本项目的威胁模型很明确：**只监听 `127.0.0.1`、单人、工作区锁定**。既然威胁模型里不存在「远端不可信调用方」，就没有必要提供一个「关掉 L5」的档位来换便利——L5 的存在成本几乎为零，而关掉它的收益是负的。
...
| 6 | 三档执行模式，默认 `auto_workspace`，**不提供完全访问** | 高权限操作需人工确认，第一阶段直接失败关闭 | 有了审批 UI/API 之后 |
...
| **执行档** | 5 档，含 **Full Access**（最少确认） | 3 档，默认 `auto_workspace`，**不提供「完全访问」** | 威胁模型差异：Zcode 面向桌面 + 手机远控 + 多人团队，本设计面向 127.0.0.1 单人 |
